...
def embed(image, message):
  ...
  # Testing to try and figure out rounding issues / inaccuracies when changing color spaces
  # test for values greater than 255 and less than 0
  # test = [item.flatten() for item in stego_image_rgb]
  # flat_test = np.asarray(test).flatten()
  # print(len(flat_test))
  # print(np.count_nonzero(flat_test < 0))
  # print(np.count_nonzero(flat_test > 255))
